...
class BackendModel(PolymorphicModel,mp.Process):
	...
	def __unicode__(self):
		return u'%s [status: %s]' % (unicode(self.__class__.__name__), unicode(self.status()))
...
class Worker(BackendModel):
	...
	def __unicode__(self):
		return u'%s [uri: %s, id: %s, status: %s]' % (unicode(self.__class__.__name__), unicode(self.machine.uri), unicode(self.identification), unicode(self.status()))
...
class Machine(BackendModel):
	...
	def __unicode__(self):
		return u'%s [uri: %s, status: %s]' % (unicode(self.__class__.__name__), unicode(self.uri), unicode(self.status()))
	...
	def configure_pyro4(self):
		...
		while not hasattr(self,'ns'):
			try:
				...
				self.ns_process = Popen('python -m Pyro4.naming -n 0.0.0.0', shell=True)
		print "[ Starting the %s on %s]" % ("manager" if self.is_manager else "worker", self.ip)
